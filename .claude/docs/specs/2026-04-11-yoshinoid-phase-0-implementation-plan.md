...
| **T6-E1** | knowledge_store 환경 배선 + CLI console script (v1.4) | `dotclaude-core/src/dotclaude_core/knowledge_store/cli.py`, `dotclaude/pyproject.toml`, `dotclaude-rag/pyproject.toml` | T6 | **Fix A (sibling-source pattern)**: `[tool.uv.sources]` 를 각 repo 의 `pyproject.toml` 에 직접 선언 (workspace root `uv.toml` 미사용 — deferred to T11). dotclaude: `dotclaude-core/rag/types` sibling path source + `knowledge` extra 추가. dotclaude-rag: `dotclaude-types` sibling path source 추가. `uv sync --extra dev --extra knowledge` 로 venv 재생성. **CLI helper**: `dotclaude-knowledge` console script (`dotclaude_core.knowledge_store.cli:app`) — typer 기반, `init` + `write-session-close` 2 sub-commands. `write-session-close` 는 minimal subset (slug + topic) 노출, 나머지 frontmatter fields 는 T8/T11 에서 확장. **Q-i Phase 1 defer 유지**: `dotclaude/src/dotclaude/cli.py` 는 건드리지 않음 (acceptance gate A4). T6-21 rename note: 이전 세션의 "T6-21 CLI facade" 태스크는 phantom 으로 제거됨 — dotclaude-core console script 방식으로 대체. **검증**: 390 passed / 2 xfailed (baseline 386+4), coverage 98.61%, cli.py 92% |
...
| 2026-04-12 | 1.4 | T6-E1 row 추가 (knowledge_store 환경 배선 + CLI console script). Fix A sibling-source pattern 선택 (workspace root 리팩터 T11 defer). dotclaude venv broken 해소 (`dotclaude-rag>=0.1.0` unsatisfiable → `[tool.uv.sources]` sibling path source). `dotclaude-knowledge` console script 구현 (`init` + `write-session-close` sub-commands, typer 기반). dotclaude-core dependencies 에 `typer>=0.12` 추가. T6-21 rename note: phantom task 제거. Q-i Phase 1 defer (A4 gate) 유지 확인. 390 passed / 2 xfailed, 98.61% coverage | T6-E1 Round 2 critic-APPROVED 실행 완료 (2026-04-12) |
